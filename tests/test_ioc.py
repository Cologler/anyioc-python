...
def assert_value_is_singleton(providers: Iterable[ServiceProvider], key: Hashable) -> None:
    for left, right in itertools.combinations_with_replacement(providers, 2):
        assert left[key] is right[key]

def assert_value_is_scoped(providers: Iterable[ServiceProvider], key: Hashable) -> None:
    for left, right in itertools.combinations_with_replacement(providers, 2):
        assert (left is right) == (left[key] is right[key])

def assert_value_is_transient(providers: Iterable[ServiceProvider], key: Hashable) -> None:
    for left, right in itertools.combinations_with_replacement(providers, 2):
...
def test_register_bind() -> None:
    provider = ServiceProvider()
    provider.register_value('k', 'value')
    provider.register_bind('b', 'k')
    assert provider['b'] == 'value'

def test_get_item_missing_service() -> None:
    provider = ServiceProvider()
...
def test_get_or_def_missing_service() -> None:
...
def test_get_many_from_multilevel() -> None:
    provider = ServiceProvider()
...
def test_get_many_missing_service() -> None:
    provider = ServiceProvider()
...
def test_resolve_with_override_kwargs() -> None:
    provider = ServiceProvider()
    provider.register_value(int, 1)

    def func(the_int_value: int) -> int:
        return the_int_value

    assert 1 == provider.resolve(func)
    assert 2 == provider.resolve(func, kwargs={'the_int_value': 2})

    def func_with_kwonly(*, the_int_value: int) -> int:
        return the_int_value
